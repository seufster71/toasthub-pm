...
public class EnhancementSvcImpl implements EnhancementSvc, ServiceProcessor {
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			enhancementDao.items(request, response);
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			enhancementDao.itemCount(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, PrefCacheUtil.getPrefText(request, "GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL").getValue(), response);
			e.printStackTrace();
		}
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			enhancementDao.delete(request, response);
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			enhancementDao.item(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, PrefCacheUtil.getPrefText(request, "GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL").getValue(), response);
			e.printStackTrace();
		}
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (inputList.containsKey(GlobalConstant.ITEMID) && inputList.get(GlobalConstant.ITEMID) != null && !"".equals(inputList.get(GlobalConstant.ITEMID))) {
				...
				enhancement.setArchive(false);
				enhancement.setLocked(false);
				request.addParam(GlobalConstant.ITEM, enhancement);
			}
			// marshall
