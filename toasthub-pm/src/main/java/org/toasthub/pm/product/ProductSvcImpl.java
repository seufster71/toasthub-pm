...
package org.toasthub.pm.product;
...
@Service("PMProductSvc")
public class ProductSvcImpl implements ProductSvc, ServiceProcessor {

	@Autowired
	@Qualifier("PMProductDao")
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			productDao.items(request, response);
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			productDao.itemCount(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			productDao.delete(request, response);
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			productDao.item(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (inputList.containsKey(GlobalConstant.ITEMID) && inputList.get(GlobalConstant.ITEMID) != null && !"".equals(inputList.get(GlobalConstant.ITEMID))) {
				...
				product.setUserId(((MyUserPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser().getId());
				request.addParam(GlobalConstant.ITEM, product);
			}
			// marshall
