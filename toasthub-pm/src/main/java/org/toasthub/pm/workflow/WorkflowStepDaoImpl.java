...
public class WorkflowStepDaoImpl implements WorkflowStepDao {
	...
	public void delete(RestRequest request, RestResponse response) throws Exception {
		if (request.containsParam(GlobalConstant.ITEMID) && !"".equals(request.getParam(GlobalConstant.ITEMID))) {
			...
			WorkflowStep workflowStep = (WorkflowStep) entityManagerDataSvc.getInstance().getReference(WorkflowStep.class,  request.getParamLong(GlobalConstant.ITEMID));
	...
	public void items(RestRequest request, RestResponse response) throws Exception {
		...
		if (request.containsParam(PMConstant.WORKFLOWID)) {
			query.setParameter("workflowId", request.getParamLong(PMConstant.WORKFLOWID));
		}
	...
	public void moveSave(RestRequest request, RestResponse response) {
		...
		List<Long> list = entityManagerDataSvc.getInstance().createQuery("SELECT x.id FROM WorkflowStep AS x WHERE x.workflow.id =:workflowId ORDER BY x.sortOrder").setParameter("workflowId", request.getParamLong(PMConstant.WORKFLOWID)).getResultList();
