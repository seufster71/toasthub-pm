...
@Service("PMWorkflowStepSvc")
public class WorkflowStepSvcImpl implements WorkflowStepSvc, ServiceProcessor {

	@Autowired
	@Qualifier("WorkflowStepDao")
	...
	public void process(RestRequest request, RestResponse response) {
		...
		case "SAVE":
			if (!request.containsParam(PrefCacheUtil.PREFFORMKEYS)) {
				List<String> forms =  new ArrayList<String>(Arrays.asList("PM_WORKFLOW_STEP_FORM"));
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			workflowStepDao.items(request, response);
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			workflowStepDao.itemCount(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			workflowStepDao.delete(request, response);
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			workflowStepDao.item(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			// save
			workflowStepDao.save(request, response);
	...
	public void moveSave(RestRequest request, RestResponse response) {
		try {
			workflowStepDao.moveSave(request, response);
			utilSvc.addStatus(RestResponse.INFO, RestResponse.SUCCESS, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_SAVE_SUCCESS",prefCacheUtil.getLang(request)), response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
